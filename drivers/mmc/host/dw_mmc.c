...
static void dw_mci_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	case MMC_POWER_UP:
		...
		if (mci_priv(slot->host) == NULL ||
		    mci_priv(slot->host)->id != MMC_SD)
			break;
...
static int dw_mci_switch_voltage(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (mci_priv(host) && mci_priv(host)->switch_voltage != NULL) {
		ret = mci_priv(host)->switch_voltage(mmc, ios);
		/* done if success */
		if (!ret)
			return ret;
	}
...
err_clk_biu:
	...
	if (mci_priv(host))
		devm_kfree(host->dev, mci_priv(host));
